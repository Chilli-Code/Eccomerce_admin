...
const fetchProducts = async () => {
  try {
    ...
    // El backend ahora devuelve { items, total }
  ...
  const handleDelete = async () => {
    try {
      if (delModal?.bulk) {
        ...
        notify.productDeleted(`${selected.length} productos`);
        setSelected([]);
      } else {
        await api.products.delete(delModal.id);
        notify.productDeleted(delModal.name);
      }
      setDelModal(null);
      fetchProducts();
    } catch {
...
const filtered = products.filter(p => {
  ...
  function SortTh({ label, field }) {
    ...
    return (
      <th className="cursor-pointer select-none hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
        onClick={() => { if (active) setSortDir(d => d === "asc" ? "desc" : "asc"); else { setSortBy(field); setSortDir("asc"); } }}>
        <span className="flex items-center gap-1">
          {label}
          <span className="text-[10px] opacity-50">{active ? (sortDir === "asc" ? "↑" : "↓") : "↕"}</span>
        </span>
      </th>
    );
  }
  ...
  function Checkbox({ checked, onChange, onClick }) {
    return (
      <label className="cursor-pointer select-none" onClick={onClick}>
        ...
        <span className={`flex h-4 w-4 items-center justify-center rounded-sm border-[1.25px] transition-colors ${checked ? "bg-primary-600 border-primary-600" : "bg-transparent border-gray-300 dark:border-gray-700"}`}>
    ...
    <div className="space-y-5">
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">{products.length} productos en total</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total productos", value: products.length,                                    sub: `${products.filter(p => p.status === "active").length} activos`,    color: "text-primary-600 dark:text-primary-400", bg: "bg-primary-50 dark:bg-primary-900/30",  icon: Package     },
          { label: "Categorías",      value: [...new Set(products.map(p => p.categoryId))].length, sub: "categorías distintas",                                             color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-900/20",  icon: Tag         },
          { label: "Sin stock",       value: products.filter(p => p.stock === 0).length,          sub: "requieren restock",                                                color: "text-red-500",                           bg: "bg-red-50 dark:bg-red-900/20",          icon: AlertCircle },
          { label: "Borradores",      value: products.filter(p => p.status === "draft").length,   sub: "sin publicar",                                                     color: "text-amber-500",                         bg: "bg-amber-50 dark:bg-amber-900/20",      icon: FileText    },
        ].map(s => (
      ...
      {selected.length > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800 rounded-xl">
          ...
          <div className="flex gap-2 ml-auto">
            <button className="btn-secondary text-xs py-1.5" onClick={() => setSelected([])}>Deseleccionar</button>
      ...
      <div className="table-wrap">
        <div className="flex flex-col md:flex-row md:items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          <div className="w-64">
        ...
        <div className="overflow-x-auto">
          <table className="table-base">
            <thead>
              <tr>
                <th className="w-10">
                  <Checkbox checked={paginated.length > 0 && selected.length === paginated.length} onChange={toggleAll} />
                </th>
            ...
            <tbody>
              {paginated.map(p => (
                <tr key={p.id} className={selected.includes(p.id) ? "bg-primary-50/50 dark:bg-primary-900/10" : ""}>
                  <td>
                    <Checkbox checked={selected.includes(p.id)} onChange={() => toggleSelect(p.id)} onClick={e => e.stopPropagation()} />
                  </td>
                  <td>
...
<td className="font-medium">{formatCOP(p.price)}</td>
                  <td>
                  ...
                  <td>
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => setDelModal(p)} className="btn-ghost p-1.5 rounded-lg text-gray-400 hover:text-red-500">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </td>
