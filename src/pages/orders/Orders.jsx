    ...
    <div className="space-y-5">
      <div className="page-header">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">{allOrders.length} orders total</p>
        </div>
        <button className="btn-secondary">Export CSV</button>
      </div>
<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
  {[
    { label: "Total órdenes",  value: allOrders.length, color: "text-gray-900 dark:text-white", bg: "bg-gray-100 dark:bg-gray-800", icon: ShoppingCart },
    { label: "Pendientes",     value: allOrders.filter(o => o.status === "pending").length,    color: "text-amber-600 dark:text-amber-400",   bg: "bg-amber-50 dark:bg-amber-900/20",   icon: Clock },
    { label: "En proceso",     value: allOrders.filter(o => o.status === "processing").length, color: "text-blue-600 dark:text-blue-400",     bg: "bg-blue-50 dark:bg-blue-900/20",     icon: Settings },
    { label: "Completadas",    value: allOrders.filter(o => o.status === "completed").length,  color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-50 dark:bg-emerald-900/20", icon: CheckCircle },
  ].map(s => (
            ...
            <tbody>
              {paginated.map(o => (
                <tr key={o.id} className="cursor-pointer" onClick={() => navigate(`/orders/${o.id.replace("#", "")}`)}>
                  ...
                  <td>
                    <div>
                      <p className="font-medium text-gray-800 dark:text-gray-200 text-sm">{o.customer}</p>
                      <p className="text-xs text-gray-400">{o.email}</p>
                    </div>
                  </td>
                  <td className="text-center">{o.items}</td>
                  <td className="font-semibold">${o.total.toFixed(2)}</td>
                  <td className="capitalize text-sm text-gray-500">{o.payment}</td>
